...
本项目是一个基于以太坊的 **NFT 拍卖行** 去中心化应用（DApp）的智能合约源代码，支持管理员作为卖家上架 NFT 进行拍卖，买家通过 **ETH** 或 **ERC20 代币** 参与竞价。拍卖合约通过 Chainlink 价格预言机将不同币种的出价统一换算为 **USD 价值** 进行比较，确保出价公平。

项目采用 **UUPS 可升级代理模式** 部署，支持合约逻辑的**热升级**而无需迁移数据。
...
- **统一 USD 计价比较**：通过 Chainlink 价格预言机将所有出价换算为 USD，确保跨币种出价可比，设置ERC20交易代币时自动获取合约美元精度
...
│   ├── AuctionHouseV1.test.ts          # 拍卖行合约测试
│   ├── MyNFT.test.ts                   # NFT 合约测试
│   └── MockPriceOracle.test.ts         # 预言机 Mock 测试
│
...
| `getETHPriceInUSD()`                      | 查询 ETH 价格                 |
| `getTokenPriceInUSD(token)`               | 查询 ERC20 价格               |
| `convertToUSD(token, amount)`             | 将代币数量换算为 USD          |

计算方式：`usdAmount = (amount × feedPrice) / 10^tokenDecimals`，tokenDecimals是美元的精度
